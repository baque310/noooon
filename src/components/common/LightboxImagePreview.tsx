  ...
  return (
    <div className="lightbox-backdrop" onClick={onClose}>
      <div className="lightbox-content justify-center items-center flex md:p-[20px] max-md:p-[5px]" onClick={(e) => e.stopPropagation()}>
        ...
        <img className="w-[80%] max-h-[100vh] max-md:flex-1" src={images[selectedImage]} alt={`Selected ${selectedImage}`} />
      </div>
    </div>
  );
};
    ...
    <>
      ...
      <div className="gallery grid grid-cols-fluid gap-2 my-2">
        {images.map((image, index) => (
          <AttachmentsImage key={index} src={image} className={`h-full w-full `} classNameImage={myClassName} />
        ))}
      </div>
    </>
  );
