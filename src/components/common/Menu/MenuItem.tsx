...
export const MenuItem = ({ to, label, icon, resource, toggleMenu, permission, number = 0 }: { number?: number, to: string, label: string, icon?: React.ReactNode, resource: PAGE_CODE | PAGE_CODE[], toggleMenu: any, permission: PERMISSION | PERMISSION[] }) => {
    const [isActive, setIsActive] = useState(false);

    return (
        <li className={`nav-item ${isActive && 'hidden'}`} onClick={() => toggleMenu(to)}>
            <Link href={to} className="group">
                <RolePageAndActionBasedComponent component={
                    (props: any) => <MenuRoleComponent
                        number={number}
