...
import { useTableGetDataRolesQuery } from "@/services/table";
import { sortBy } from "lodash";
import { searchArray } from "@/utils/searchArray";
import { useSession } from "next-auth/react";
import { rolesData } from "@/services/types/constant";
...
const Page = () => {
  ...
  const { currentData: TableGetDataRoles, isFetching: isFetchingTableGetDataRoles } = useTableGetDataRolesQuery();

  const [dataById, setDataById] = useState<AdminDataResponse | undefined>();
  const [dataAdminCompoundById, setDataAdminCompoundById] = useState<AdminDataResponse | undefined>();

  const [AdminGetDataById, { currentData: DataAdminGetDataById, isFetching }] = useLazyAdminGetDataByIdQuery();
  const isCompound = session.data?.user.Compound;
  useEffect(() => {
    if (id) {
  ...
  const [AdminCompoundUpdate, { isLoading: isLoadingAdminCompoundUpdate }] = useAdminCompoundUpdateMutation();
  ...
  const handleSubmit = async (values: FormValues, { setSubmitting, resetForm }: FormikHelpers<FormValues>) => {
    try {
      ...
      if (id) {
        isCompound ? await AdminCompoundUpdate({ id: String(id), body: formData }).unwrap() : await AdminUpdate({ id: String(id), body: formData }).unwrap();
      }

      toast.success(t("updated-successfully"), { autoClose: 30000 });
      resetForm();
      router.back();
    } catch (error: any) {
  ...
  return (
    <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%]">
      <BackButton title={t("update-role")} />
      {isFetching || isFetchingTableGetDataRoles ? (
        <LoadingForm />
      ) : (
        <Formik<FormValues>
          initialValues={{
            ...
            password: undefined,
            isActive: dataById?.isActive ?? "false",
            roles: Array.isArray(dataById?.roles) ? [...dataById.roles] : [],
            dataAvailableGroupPrivileges: Array.isArray(dataById?.roles) ? [...dataById.roles] : [],
          }}
          onSubmit={handleSubmit}>
          {(props: FormikProps<FormValues>) => (
            <Form className="p-4 flex flex-col gap-4">
              <div className="Card flex flex-col gap-1">
                <div className="text-base font-semibold text-black dark:text-white-dark mb-2">{t("role-information")}</div>
                <div className="px-1 flex items-center justify-between bg-primary rounded-lg py-1">
                  {TabList.map((item, index) => (
                    <button
                      ...
                      <div className="font-['Loew Next Arabic'] text-center font-bold uppercase tracking-tight text-white">{t(item.label)}</div>
                    </button>
                  ))}
                </div>
                <InputForm formikProps={props} name="search" title="" placeholder={t("search")} />
                <div className="h-[calc(100vh-320px)] overflow-y-auto">
                  {sortBy(searchArray<Roles>(isCompound ? rolesData : TableGetDataRoles, props.values.search, ["resource_ar"]), "resource_ar")
                    .filter(
                      (it: Roles) =>
                        selected === "all" ||
                    ...
                    .map((privilege: Roles) => (
                      <div key={privilege.resource} className="p-1 w-full flex flex-col">
                        <label className="inline-flex items-center">
                          ...
                          />
                          <span className="peer-checked:text-primary">{t(privilege.resource_ar)}</span>
                        </label>
                        <div className="my-2 flex flex-col mx-4 gap-1">
                          {privilege.rolesString?.map((permission, index) => (
                            <label key={index} className="inline-flex w-[180px]">
                              ...
                              />
                              <span className="peer-checked:text-primary">{t(permission)}</span>
                            </label>
                          ))}
                        </div>
                      </div>
                    ))}
                  {sortBy(searchArray<Roles>(isCompound ? rolesData : TableGetDataRoles, props.values.search, ["resource_ar"]), "resource_ar").length === 0 && (
                    ...
                    component={(props) => {
                      return (
                        <ButtonForm
                          ...
                          title={t("save")}
                          isLoading={isLoadingAdminUpdate || isLoadingAdminCompoundUpdate}
                        />
                      );
                    }}
                    resource={"admin"}
