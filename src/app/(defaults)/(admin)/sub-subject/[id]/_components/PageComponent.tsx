"use client";

import React, { useState } from "react";
import { LoadingForm } from "@/components/Form/loadingForm";
import { BackButton } from "@/components/common/BackButton";
import { ItemList } from "@/components/common/ItemList";

import { getTranslation } from "@/ni18n/i18n";
import { useLazySubSubjectGetDataByIdQuery, useSubSubjectRemoveMutation } from "@/services/admin/SubSubject";
import { useParams, useRouter } from "next/navigation";
import { useEffect } from "react";
import { toast } from "react-toastify";
import { ArrowIcons } from "@/components/common/icons/Actions";
import moment from "moment";
import DeleteModel from "@/components/Model/DeleteModel";
import CreateComponent from "../../_components/CreateComponent";

const PageComponent = () => {
  const { t } = getTranslation();
  const router = useRouter();
  const params = useParams();
  const { id } = params;
  const [SubSubjectGetDataById, { currentData: data, isFetching }] = useLazySubSubjectGetDataByIdQuery();
  const [SubSubjectRemove, { isLoading: isLoadingSubSubjectRemove }] = useSubSubjectRemoveMutation();

  useEffect(() => {
    if (id) {
      SubSubjectGetDataById({ id: String(id) }).then((data) => {
        if (!data.data) {
          router.back();
        }
      });
    }
  }, [id]);
  const handleRemove = async () => {
    try {
      await SubSubjectRemove({ id: String(id) }).unwrap();
      toast.success(t("common.deleted-successfully"), { autoClose: 15000 });
      router.back();
    } catch (error: any) {
      console.error("Failed to operation :", error);
      if (error && error.message) {
        if (error.message === 'Foreign key constraint failed on the field. More details: {"modelName":"SubSubject","field_name":"subSubjectId"}') {
          return toast.error(t("SubSubjectPage.this-SubSubject-connected-with-other-data"), { autoClose: 15000 });
        }
        return toast.error(t(error.message), { autoClose: 15000 });
      }
      toast.error(error, { autoClose: 15000 });
    }
  };

  const [open, setOpen] = useState(false);
  const [openDelete, setOpenDelete] = useState(false);
  return (
    <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
      <BackButton title={t("SubSubjectPage.SubSubjectInformation")} />

      {isFetching ? (
        <LoadingForm />
      ) : (
        <>
          <div className="CardDetails internalMenu ">
            <ItemList title={t("SubSubjectPage.name")} value={String(data?.name)} />
            <ItemList title={t("common.updatedAt")} value={moment(data?.updatedAt).format("YYYY-MM-DD hh:mm:ss A")} />
            <ItemList title={t("common.createdAt")} value={moment(data?.createdAt).format("YYYY-MM-DD hh:mm:ss A")} />
          </div>

          <div className="text-sm font-semibold text-black dark:text-white-dark  mt-2 mb-1 ">{t("common.settings")}</div>
          <div className="CardDetails internalMenu ">
            <ItemList
              props={{
                onClick: () => {
                  setOpen(true);
                },
              }}
              title={t("SubSubjectPage.update-info")}
              value={<ArrowIcons className="rtl:rotate-180 text-[#000]/50" />}
            />
            <ItemList
              props={{
                onClick: () => {
                  setOpenDelete(true);
                },
              }}
              title={<div className="text-danger">{t("common.delete")}</div>}
              value={<ArrowIcons className="rtl:rotate-180 text-danger/50" />}
            />
          </div>
        </>
      )}
      <CreateComponent open={open} setOpen={setOpen} />
      <DeleteModel
        description={t("SubSubjectPage.Are-you-sure-you-want-to-delete-this-SubSubject")}
        title={t("SubSubjectPage.DeleteSubSubject")}
        open={openDelete}
        setOpen={setOpenDelete}
        handleRemove={handleRemove}
        isLoading={isLoadingSubSubjectRemove}
        name={data?.name ?? ""}
      />
    </div>
  );
};

export default PageComponent;
