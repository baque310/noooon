...
import { useGalleryGetDataQuery } from "@/services/admin/Gallery";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useGalleryGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/gallery?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("GalleryPage.Gallery")}</div>
...
export default withRole(TableComponent, "gallery", ["read-any", "read-own"]);
