...
import { useLazyGalleryGetDataByIdQuery, useGalleryRemoveMutation } from "@/services/admin/Gallery";
...
const PageComponent = () => {
    ...
    const [GalleryRemove, { isLoading: isLoadingGalleryRemove }] = useGalleryRemoveMutation()

    useEffect(() => {
        if (id) {
            GalleryGetDataById({ id: String(id) })
                .then((data) => {
                    if (!data.data) {
                        router.back();
                    }
    ...
    return (
        <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
...

            {
                isFetching ? <LoadingForm /> : <>
                    ...
                    <div className='CardDetails internalMenu '>
                        <ItemList title={t('GalleryPage.title')} value={String(data?.title)} /> 
            ...
            <DeleteModel
                description={t('GalleryPage.Are-you-sure-you-want-to-delete-this-Gallery')}
