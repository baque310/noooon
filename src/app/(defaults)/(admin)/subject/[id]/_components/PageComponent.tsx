...
import { useLazySubjectGetDataByIdQuery, useSubjectRemoveMutation } from "@/services/admin/Subject";
...
const PageComponent = () => {
    ...
    const [SubjectRemove, { isLoading: isLoadingSubjectRemove }] = useSubjectRemoveMutation()
    ...
    const handleRemove = async () => {
        try {
            await SubjectRemove({ id: String(id) }).unwrap();
            toast.success(t('common.deleted-successfully'), { autoClose: 15000 });
            router.back();
        } catch (error: any) {
            console.error('Failed to operation :', error);
            if (error && error.message) {
                if (error.message === 'Foreign key constraint failed on the field. More details: {"modelName":"Subject","field_name":"subjectId"}') {
                    return toast.error(t('SubjectPage.this-Subject-connected-with-other-stage'), { autoClose: 15000 });
                }
    ...
    return (
        <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
            <BackButton title={t('SubjectPage.SubjectInformation')} />

            {
                isFetching ? <LoadingForm /> : <>
                    <div className='CardDetails internalMenu '>
                        <ItemList title={t('SubjectPage.name')} value={String(data?.name)} />
                    ...
                    <div className='CardDetails internalMenu '>
                        <ItemList
                            ...
                            title={t('SubjectPage.update-info')}
            ...
            <DeleteModel
                description={t('SubjectPage.Are-you-sure-you-want-to-delete-this-Subject')}
                title={t('SubjectPage.DeleteSubject')}
                open={openDelete}
                setOpen={setOpenDelete}
                handleRemove={handleRemove}
                isLoading={isLoadingSubjectRemove}
                name={data?.name ?? ""}
            />
