...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useOtherPaymentGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/otherPayment?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4 rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("OtherPaymentPage.otherPayment")}</div>
              ...
              component={(props) => {
                return (
                  <button
                    ...
                    onClick={() => {
                      router.push("/otherPayment/createOrUpdate");
                    }}>
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/otherPayment/${item.record.id}`);
            }}
            fetching={isFetching}
              ...
              },
              {
                ...
                render: ({ paymentMethod }: any) => (
                  <div className="flex gap-2 px-[2px]">
                    <div className={` rounded-md p-1 text-center bg-success/20 text-success `}>{t(paymentMethod as any)}</div>
                  </div>
                ),
              },
              {
      ...
      {/* <ChangeStatusOtherPaymentComponent
