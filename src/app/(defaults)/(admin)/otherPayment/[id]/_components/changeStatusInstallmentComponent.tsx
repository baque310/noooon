...
import { PaymentMethod, Status } from "@/services/admin/installmentPayment";
import { SelectForm } from "@/components/Form/SelectForm";
import { InputForm } from "@/components/Form/inputForm";
import { IOtherPayment, OtherPaymentStatusPayload, useOtherPaymentChangeStatusMutation } from "@/services/admin/other-payment";

// Correct form values type
export interface FormValues extends OtherPaymentStatusPayload {
  status: Status;
}

const ChangeStatusOtherPaymentComponent = ({
  open,
  setOpen,
  data,
  outstandingAmount,
  ...
  // const [OtherPaymentPaymentCreate, { isLoading: isLoadingOtherPaymentPaymentCreate }] = useOtherPaymentPaymentCreateMutation();
  const [OtherPaymentPaymentUpdate, { isLoading: isLoadingOtherPaymentPaymentUpdate }] = useOtherPaymentChangeStatusMutation();
  ...
  const handleSubmit = async (values: FormValues, { setSubmitting, resetForm }: FormikHelpers<FormValues>, setOpen: any) => {
    try {
      ...
      //   await OtherPaymentPaymentCreate({
      ...
      // } else {
      //   console.log("other");
      //   await OtherPaymentPaymentUpdate({
      //     id: data.id,
            ...
            <Form className="flex flex-col gap-4">
              {data?.isPaid !== "partial" && (
                <SelectForm
                  formikProps={props}
                  name="status"
                  title={t("OtherPaymentPage.status")}
              ...
              <SelectForm
                ...
                placeholder={t("OtherPaymentPage.enter-paymentMethod")}
                options={[
              ...
              <div className="flex flex-row-reverse gap-2">
                <ButtonForm
                  ...
                  title={t("common.save")}
                  isLoading={isLoadingOtherPaymentPaymentUpdate}
                />
              </div>
            </Form>
          )}
...
export default ChangeStatusOtherPaymentComponent;
