...
import { useBusGetDataQuery } from "@/services/admin/bus";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useBusGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/bus?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("BusPage.bus")}</div>
...
export default withRole(TableComponent, "bus", ["read-any", "read-own"]);
