...
import { useLazyBusGetDataByIdQuery, useBusRemoveMutation } from "@/services/admin/bus";
...
const PageComponent = () => {
    ...
    const [BusRemove, { isLoading: isLoadingBusRemove }] = useBusRemoveMutation()
    ...
    const handleRemove = async () => {
        try {
            await BusRemove({ id: String(id) }).unwrap();
    ...
    return (
        <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
            <BackButton title={t('BusPage.BusInformation')} />

            {
                isFetching ? <LoadingForm /> : <>
                    <AttachmentsImage className="my-2 h-44" src={String(data?.photo)} />
                    <div className='CardDetails internalMenu '>
                        <ItemList title={t('BusPage.fullName')} value={String(data?.fullName)} />
                        {/* <ItemList title={t('BusPage.Username')}
                            value={String(data?.User?.username)}
                            isCopyToClipboard
                        /> */}
                        <ItemList title={t('BusPage.address')} value={String(data?.address)} />
                        <ItemList title={t('BusPage.phone1')} value={String(data?.phone1)} />
                        <ItemList title={t('BusPage.phone2')} value={String(data?.phone2)} />
                        <ItemList title={t('BusPage.carColor')} value={String(data?.carColor)} /> 
                        <ItemList title={t('BusPage.carNumber')} value={String(data?.carNumber)} /> 
                        <ItemList title={t('BusPage.carType')} value={String(data?.carType)} /> 
                    ...
                    <div className='CardDetails internalMenu '>
                        <ItemList
                            props={{
                                onClick: () => {
                                    router.push(`/bus/createOrUpdate?id=${id}`)
                                }
                            }}
                            title={t('BusPage.update-info')}
            ...
            <DeleteModel
                description={t('BusPage.Are-you-sure-you-want-to-delete-this-Bus')}
                title={t('BusPage.DeleteBus')}
                open={openDelete}
                setOpen={setOpenDelete}
                handleRemove={handleRemove}
                isLoading={isLoadingBusRemove}
                name={data?.fullName ?? ""}
            />
