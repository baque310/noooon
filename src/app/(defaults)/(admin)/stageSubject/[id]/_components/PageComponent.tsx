...
import { useLazyStageSubjectGetDataByIdQuery, useStageSubjectRemoveMutation } from "@/services/admin/StageSubject";
...
const PageComponent = () => {
  ...
  const [StageSubjectRemove, { isLoading: isLoadingStageSubjectRemove }] = useStageSubjectRemoveMutation();

  useEffect(() => {
    if (id) {
      StageSubjectGetDataById({ id: String(id) }).then((data) => {
        if (!data.data) {
          router.back();
        }
      });
  ...
  const handleRemove = async () => {
    try {
      await StageSubjectRemove({ id: String(id) }).unwrap();
  ...
  return (
    <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
      <BackButton title={t("StageSubjectPage.StageSubjectInformation")} />
      ...
      {isFetching ? (
        ...
        <>
          <div className="CardDetails internalMenu ">
            <ItemList title={t("StageSubjectPage.SubjectName")} value={String(data?.Subject.name)} />
            <ItemList title={t("StageSubjectPage.StageName")} value={t(data?.Stage.name ?? ("" as any))} />
            <ItemList title={t("StageSubjectPage.ClassName")} value={String(data?.Class.name ?? "")} />
          ...
          <div className="CardDetails internalMenu ">
            {/* <ItemList
                            ...
                            title={t('StageSubjectPage.update-info')}
      ...
      <DeleteModel
        description={t("StageSubjectPage.Are-you-sure-you-want-to-delete-this-StageSubject")}
        title={t("StageSubjectPage.DeleteStageSubject")}
        open={openDelete}
        setOpen={setOpenDelete}
        handleRemove={handleRemove}
        isLoading={isLoadingStageSubjectRemove}
