...
const PageComponent = () => {
    ...
    useEffect(() => {
        if (id) {
            ExamResultsGetDataById({ id: String(id) })
                .then((data) => {
                    if (!data.data) {
                        router.back();
                    }
...

            {
                isFetching ? <LoadingForm /> : <>
                    <div className='CardDetails internalMenu '>
                        ...
                        <ItemList title={t('ExamResultsPage.StageName')} value={data?.ExamSection?.Exam?.StageSubject?.Stage?.name && t(data?.ExamSection.Exam.StageSubject.Stage.name as any)} />
