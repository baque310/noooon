...
import { useLazyTeacherSubjectGetDataByIdQuery, useTeacherSubjectRemoveMutation } from "@/services/admin/TeacherSubject";
...
const PageComponent = () => {
  ...
  useEffect(() => {
    if (id) {
      TeacherSubjectGetDataById({ id: String(id) }).then((data) => {
        if (!data.data) {
          router.back();
        }
      });
  ...
  const handleRemove = async () => {
    try {
      await TeacherSubjectRemove({ id: String(id) }).unwrap();
  ...
  return (
    <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
      <BackButton title={t("TeacherSubjectPage.TeacherSubjectInformation")} />
      ...
      {isFetching ? (
        ...
        <>
          <div className="CardDetails internalMenu ">
            <ItemList title={t("TeacherSubjectPage.SubjectName")} value={String(data?.StageSubject.Subject.name)} />
            <ItemList title={t("TeacherSubjectPage.StageName")} value={t(data?.StageSubject.Stage.name ?? ("" as any))} />
            <ItemList title={t("TeacherSubjectPage.ClassName")} value={String(data?.StageSubject.Class.name ?? "")} />
          ...
          <div className="CardDetails internalMenu ">
            <ItemList
              ...
              title={t("TeacherSubjectPage.update-info")}
      ...
      <DeleteModel
        ...
        name={data?.StageSubject.Subject.name ?? ""}
      />
    </div>
  );
};
