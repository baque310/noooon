      ...
      {isFetching || isFetchingSettings ? (
        ...
        <Formik<FormValues>
          ...
          {(props: FormikProps<any>) => {
            // Move filteredStudents inside Formik to access props.values
            const filteredStudents = useMemo(() => {
              ...
              if (studentSearch.trim()) {
                filtered = students.filter((student) => student.fullName.toLowerCase().includes(studentSearch.toLowerCase()));
              }

              // Sort: selected students first, then unselected
              return [...filtered]?.sort((a, b) => {
                const aSelected = props.values?.studentIds?.includes(a.id) ? 1 : 0;
                const bSelected = props.values?.studentIds?.includes(b.id) ? 1 : 0;
                return bSelected - aSelected;
              });
            }, [students, studentSearch, props.values?.studentIds]);
                            ...
                            {filteredStudents.length > 0 ? (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {filteredStudents.map((student, index) => {
                                  const isSelected = props.values.studentIds.includes(student.id);
                                  return (
                                    <div
                                      key={student.id}
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        />
                                        {isSelected && (
                                          <span className="ml-2 px-2 py-1 text-xs font-medium text-blue-700 dark:text-blue-300 bg-blue-100 dark:bg-blue-900/50 rounded-full">
                              ...
                              <div className="text-center py-12">
                                ...
                                <p className="text-gray-500">
                                  {studentSearch
                                    ? t("TeacherLessonsPage.try-adjusting-your-search-terms")
                                    : t("TeacherLessonsPage.no-students-available-for-the-selected-criteria")}
                                </p>
                              </div>
                            )}
                          </div>
