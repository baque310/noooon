...
const CreateComponent = ({
  ...
  const { id } = params;
  const [SectionGetDataById, { currentData: data, isFetching }] =
    useLazySectionGetDataByIdQuery();
  useEffect(() => {
    if (id) {
  ...
  const [SectionCreate, { isLoading: isLoadingSectionCreate }] =
    useSectionCreateMutation();
  const [SectionUpdate, { isLoading: isLoadingSectionUpdate }] =
    useSectionUpdateMutation();
  const { currentData: classData, isFetching: isFetchingClass } =
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (error) {
        if (
          error.message == "section already exists" ||
          error.message == "name already exist"
        ) {
          return toast.error(t("SectionPage.name-already-exists"), {
            autoClose: 30000,
          });
        }
