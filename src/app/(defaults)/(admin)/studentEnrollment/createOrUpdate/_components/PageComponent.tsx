  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (error) {
        if (
          error.message ==
          'Resource already exists. More details: {"modelName":"StudentEnrollment","target":"student_enrollments_schoolYearId_studentId_key"}'
        ) {
          return toast.error(
            t("StudentEnrollmentPage.schoolYear-already-exists"),
            { autoClose: 30000 }
          );
        }
    ...
    <>
      <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%]">
        <BackButton
          title={t(
            id
              ? "StudentEnrollmentPage.update-info"
              : "StudentEnrollmentPage.add"
          )}
        />
                          ...
                          >
                            <CheckBoxForm
                              ...
                              props={{
                                ...
                                value: props.values.students.some(
                                  (it: any) => it?.studentId == item.id
                                ),
                                onChange: (e) => {
                                  if (e.target.checked) {
