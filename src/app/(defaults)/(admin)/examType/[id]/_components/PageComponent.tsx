...
import { useLazyExamTypeGetDataByIdQuery, useExamTypeRemoveMutation } from "@/services/admin/ExamType";
...
const PageComponent = () => {
    ...
    const [ExamTypeRemove, { isLoading: isLoadingExamTypeRemove }] = useExamTypeRemoveMutation()
    ...
    const handleRemove = async () => {
        try {
            await ExamTypeRemove({ id: String(id) }).unwrap();
            toast.success(t('common.deleted-successfully'), { autoClose: 15000 });
            router.back();
        } catch (error: any) {
            console.error('Failed to operation :', error);
            if (error && error.message) {
                if (error.message === 'Foreign key constraint failed on the field. More details: {"modelName":"ExamType","field_name":"examTypeId"}') {
                    return toast.error(t('ExamTypePage.this-ExamType-connected-with-other-stage'), { autoClose: 15000 });
                }
    ...
    return (
        <div className="mx-auto my-0 max-md:max-w-[100%] md:max-w-[50%] mb-20">
            <BackButton title={t('ExamTypePage.ExamTypeInformation')} />

            {
                isFetching ? <LoadingForm /> : <>
                    <div className='CardDetails internalMenu '>
                        <ItemList title={t('ExamTypePage.name')} value={String(data?.name)} />
                    ...
                    <div className='CardDetails internalMenu '>
                        <ItemList
                            ...
                            title={t('ExamTypePage.update-info')}
            ...
            <DeleteModel
                description={t('ExamTypePage.Are-you-sure-you-want-to-delete-this-ExamType')}
                title={t('ExamTypePage.DeleteExamType')}
                open={openDelete}
                setOpen={setOpenDelete}
                handleRemove={handleRemove}
                isLoading={isLoadingExamTypeRemove}
                name={data?.name ?? ""}
            />
