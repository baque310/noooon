...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useExamTypeGetDataQuery({
    ...params,
  }); 
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/examType?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("ExamTypePage.ExamTypes")}</div>
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/examType/${item.record.id}`);
            }}
            fetching={isFetching}
...
export default withRole(TableComponent, "exam_type", ["read-any", "read-own"]);
