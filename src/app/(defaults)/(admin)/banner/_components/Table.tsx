...
import { useBannerGetDataQuery } from "@/services/admin/Banner";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useBannerGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/banner?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("BannerPage.banners")}</div>
              ...
              component={(props) => {
                return (
                  <button
                    ...
                    onClick={() => {
                      router.push("/banner/createOrUpdate");
                    }}>
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/banner/${item.record.id}`);
            }}
            fetching={isFetching}
...
export default withRole(TableComponent, "banner", ["read-any", "read-own"]);
