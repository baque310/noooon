...
import { useGuidanceGetDataQuery } from "@/services/admin/Guidance";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useGuidanceGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/guidance?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("GuidancePage.guidances")}</div>
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/guidance/${item.record.id}`);
            }}
            fetching={isFetching}
...
export default withRole(TableComponent, "guidance", ["read-any", "read-own"]);
