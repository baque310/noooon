...
import { useVideoGetDataQuery } from "@/services/admin/video";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useVideoGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/video?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("VideoPage.videos")}</div>
              ...
              component={(props) => {
                return (
                  <button
                    ...
                    onClick={() => {
                      router.push("/video/createOrUpdate");
                    }}>
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/video/${item.record.id}`);
            }}
            fetching={isFetching}
...
export default withRole(TableComponent, "video", ["read-any", "read-own"]);
