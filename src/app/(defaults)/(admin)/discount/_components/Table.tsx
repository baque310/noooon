...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useAdminDiscountGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/discount?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    ...
    >
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("DiscountPage.discounts")}</div>
              ...
              component={(props) => {
                return (
                  <button
                    ...
                    onClick={() => {
                      router.push("/discount/createOrUpdate");
                    }}
                  >
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/discount/${item.record.id}`);
            }}
            fetching={isFetching}
...
export default withRole(TableComponent, "discount", ["read-any", "read-own"]);
