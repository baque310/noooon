      ...
      if (id) {
        ...
        await SectionScheduleCreate({
          SectionSchedules: SectionSchedules?.filter((day): day is any => day !== null) // Filter out null values
            .flatMap(day => day?.SectionSchedules).filter(it => !!it)
        }).unwrap()
      }
            ...
            {(props: FormikProps<any>) => (
              <Form className={"px-4 flex flex-col gap-4"}>
                ...
                <div className="Card flex flex-col gap-1">
                  ...
                  />}
                  {props?.values?.classId && <SelectForm
                    formikProps={props}
                    name={`sectionId`}
                ...
                {!id ? <>
...

                  {
                    ...
                    <div className="mt-[2px] w-full p-1 text-sm text-danger">{t((props.errors.days ?? "") as any)}</div>
                  }
                          ...
                          <button type="button"
                            ...
                            onClick={() => {
                              const remainingDays = daysArray.filter(
                                (day) => !props.values.days.some((matrixDay: any) => matrixDay.value === day.value)
                              );
