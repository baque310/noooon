...
const PersonalInfoSection: React.FC<PersonalInfoSectionProps> = ({ data }) => {
  ...
  const CopyableInfoCard: React.FC<{
    ...
    copyable?: boolean;
    fieldName?: string;
  }> = ({ icon, title, value, gradientFrom, gradientTo, borderColor, iconBgFrom, iconBgTo, colSpan = "", copyable = false, fieldName = "" }) => {
            ...
            {copyable && value && (
              <button
                ...
                {isCopied ? (
                  ...
                  <svg className="w-3.5 h-3.5 text-gray-600 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-gray-300" fill="currentColor" viewBox="0 0 20 20">
  ...
  const personalInfoItems = [
    ...
    },
    {
      ...
      value: data?.gender ? t(`common.${data.gender.toLowerCase()}` as any) : null,
    ...
    <div className="relative group">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-3xl border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-xl shadow-xl overflow-hidden">
        ...
        <div className="relative bg-gradient-to-r from-blue-500/5 via-purple-500/5 to-pink-500/5 dark:from-blue-400/10 dark:via-purple-400/10 dark:to-pink-400/10 p-8 border-b border-gray-200/50 dark:border-gray-700/50">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t("StudentPage.infoStudent")}</h2>
          <p className="text-gray-600 dark:text-gray-400">{t("StudentPage.studentPersonalDetails")}</p>
        </div>
