...
const TabButton = ({ isSelected, onClick, label, icon, color }: { isSelected: boolean; onClick: () => void; label: string; icon: React.ReactNode; color: string }) => (
  <button
    onClick={onClick}
    ...
    <span className="flex items-center justify-center gap-2 w-full">
      <div
        className={`w-5 h-5 rounded-lg flex items-center justify-center transition-all duration-300 ${
          isSelected ? "bg-white/40 text-gray-800 dark:text-gray-100" : `bg-${color}-100 dark:bg-${color}-900/30 text-${color}-600 dark:text-${color}-400`
        }`}>
    ...
    <div className="flex justify-center mb-8 w-full">
      <div className="relative w-full bg-gradient-to-r from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-1.5 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-600">
        ...
        <div className="relative flex w-full">
          {tabs.map((tab) => (
            <TabButton key={tab.key} isSelected={selected === tab.key} onClick={() => setSelected(tab.key)} label={t(tab.labelKey as any)} icon={tab.icon} color={tab.color} />
          ))}
        </div>
      </div>
    </div>
