...
const ExcelAdd = () => {
  ...
  const [isLoadingStudentUpdate, setIsLoadingStudentUpdate] = React.useState(false);
  const [StudentMultiStudentsForExcel, { isLoading, data }] = useStudentMultiStudentsForExcelMutation();
    ...
    try {
      ...
      await StudentMultiStudentsForExcel({
        students: studentsData.map((item) => ({
          ...
          gender: item.gender === "male" ? "Male" : item.gender === "female" ? "Female" : item.gender,
          phone1: item.phone1,
          birth: !!item.birth ? item.birth : undefined,
          enrollmentDate: !!item.enrollmentDate ? item.enrollmentDate : undefined,
            ...
            <div className="relative p-8">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">{t("StudentPage.excel-upload-description")}</p>
                </div>
              </div>

              <UploadFileForm accept=".xlsx, .xls" formikProps={props} name={"file"} title={t("StudentPage.upload-excel")} placeholder={""} />
            </div>
          </div>
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-gray-900 dark:via-green-900/20 dark:to-emerald-900/20 rounded-2xl border border-green-200 dark:border-gray-700 shadow-lg">
                  <div className="relative p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-green-800 dark:text-green-300">{t("ParentPage.successCount")}</h3>
                        <p className="text-3xl font-bold text-green-600 dark:text-green-400">{data?.successCount || 0}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-red-50 via-rose-50 to-pink-50 dark:from-gray-900 dark:via-red-900/20 dark:to-rose-900/20 rounded-2xl border border-red-200 dark:border-gray-700 shadow-lg">
                  <div className="relative p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-red-800 dark:text-red-300">{t("ParentPage.errorCount")}</h3>
                        <p className="text-3xl font-bold text-red-600 dark:text-red-400">{data?.errorCount || 0}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-gray-900 dark:via-green-900/10 dark:to-emerald-900/10 rounded-2xl border border-green-200 dark:border-gray-700 shadow-lg">
                  <div className="relative p-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-green-800 dark:text-green-300">{t("StudentPage.Successfully Added Students")}</h3>
                    </div>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {data.success.map((item, index) => (
                        <div key={index} className="flex items-center gap-3 p-3 bg-white/60 dark:bg-gray-800/60 rounded-xl border border-green-100 dark:border-gray-600">
                          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                          <span className="text-green-700 dark:text-green-300 font-medium">{item.fullName}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-red-50 via-rose-50 to-pink-50 dark:from-gray-900 dark:via-red-900/10 dark:to-rose-900/10 rounded-2xl border border-red-200 dark:border-gray-700 shadow-lg">
                  <div className="relative p-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-red-800 dark:text-red-300">{t("StudentPage.Failed Entries")}</h3>
                    </div>
                    <div className="space-y-3 max-h-60 overflow-y-auto">
                      {data.errors.map((error, index) => (
                        <div key={index} className="p-4 bg-white/60 dark:bg-gray-800/60 rounded-xl border border-red-100 dark:border-gray-600">
                          <div className="flex items-center gap-3 mb-2">
                            <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                            <span className="font-semibold text-red-700 dark:text-red-300">{error.student.fullName}</span>
                          </div>
                              ...
                              {error.error.includes("students_phone1_key")
                                ...
                                : error.error.includes("students_phone2_key")
                                  ...
                                  : error.error.includes("students_email_key")
                                    ...
                                    : error.error.includes("Unique constraint failed on the constraint:")
                                      ? error.error.split("Unique constraint failed on the constraint:")[1]
                                      : error.error}
                            </p>
                          </div>
                        </div>
