...
export const ConnectedStudentWithParent = ({ data }: { data: any }) => {
  const { t } = getTranslation();
  const [open, setOpen] = useState(false);
  const [openDisconnect, setOpenDisconnect] = useState(false);

  const [StudentConnectParent, { isLoading: isLoadingStudentConnectParent }] =
    useStudentConnectParentMutation();
  const [
  ...
  return (
    ...
    >
      ...
      {open && (
        <Model
          title={t("StudentPage.ConnectedStudentWithParent")}
          open={open}
          setOpen={setOpen}
        >
          <Formik<FormValues>
      ...
      {openDisconnect && (
        <Model
          title={t("StudentPage.disconnectParent")}
          open={openDisconnect}
          ...
          <div className="flex justify-end mt-4">
            <ButtonForm
              props={{
                type: "button",
                className: "w-full",
                onClick: handleSubmitDisconnect,
              }}
