...
import { useManagerBannerGetDataQuery } from "@/services/Manager/Banner";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useManagerBannerGetDataQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/managerManagerBanner?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
  ...
  return (
    <div className={`m-4    rtl:transition-[left] ltr:transition-[right] duration-1000`}>
      <div className={"flex justify-between max-md:flex-col gap-2 "}>
        <div className="text-xl uppercase ">{t("ManagerBannerPage.ManagerBanner")}</div>
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/managerBanner/${item.record.id}`);
            }}
            fetching={isFetching}
