...
import { useComplaintGetDataForManagerQuery } from "@/services/admin/complaint";
...
const TableComponent = () => {
  ...
  const { isFetching, currentData: data } = useComplaintGetDataForManagerQuery({
    ...params,
  });
  ...
  const handleSearch = (value?: string) => {
    if (search != Search) {
      ...
      router.push(`/complaint?${allParams.toString()}`);
      setPageNumber(1);
    }
  };
        ...
        {isMounted && (
          <DataTable
            onRowClick={async (item) => {
              router.push(`/complaint/${item.record.id}`);
            }}
            fetching={isFetching}
